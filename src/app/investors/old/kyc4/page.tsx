    ...
    return (
        <>
            ...
            <section className="progress-container" style={{ textAlign: 'center', marginBottom: '4rem' }}>
                <h2>KYC & Accreditation {status === 'pending' ? 'Status' : 'Progress'}</h2>
                <div className="progress-bar">
                    <div className="progress-line"></div>
                    <div className="progress-fill" style={{ width: status === 'approved' ? '100%' : '75%' }}></div>
                    ...
                    <div className={`progress-step ${status === 'pending' ? 'active' : 'completed'}`}>
                        <div className="progress-circle">3</div>
                        <div className="progress-label">Accreditation</div>
                    </div>
                    <div className={`progress-step ${status === 'approved' ? 'completed' : (status === 'declined' ? 'active' : '')}`}>
                        <div className="progress-circle">4</div>
                        <div className="progress-label">Review & Approval</div>
                    </div>
                </div>
                {status === 'pending' && <p style={{ fontSize: '1.2rem' }}>Your KYC is 75% complete. Final accreditation review pending.</p>}
                {status === 'approved' && <p style={{ fontSize: '1.2rem' }}>Congratulations! Your KYC & accreditation has been fully approved.</p>}
                {status === 'declined' && <p style={{ fontSize: '1.2rem' }}>Your submission requires additional information before approval.</p>}
            </section>
